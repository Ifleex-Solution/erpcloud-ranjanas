...
<div class="row">
    <div class="col-sm-15">
        <div class="panel panel-default" style="background-color:#B2BEB5">
            <h3 style="margin-left: 30px;">Counter Sales Summary</h3>
            <br />
            <div class="panel-body" style="margin-left: 120px;">
                ...
                <button type="button" id="btn-filter" class="btn btn-success" onclick="onFilterButtonClick2()">
                   Generate Report2
                </button>
            </div>
        </div>
...
<script>
    function onFilterButtonClick() {
        $.ajax({
            type: "post",
            url: $('#baseUrl2').val() + 'report/report/countersales_report',
            data: {
            ...
            success: function(data1) {
                ...
                console.log(datas)
                if (datas != null) {
                    window.open(`generate_countersalesreport`, '_blank');
    ...
    function onFilterButtonClick2() {
        $.ajax({
            type: "post",
            url: $('#baseUrl2').val() + 'report/report/countersales_report2',
            data: {
                from_date: $('#from_date').val(),
                to_date: document.getElementById('single_date_checkbox').checked ? $('#from_date').val() : $('#to_date').val(),
                empid: $('#empid').val(),
            ...
            success: function(data1) {
                ...
                console.log(datas)
                if (datas != null) {
                    window.open(`generate_countersales2`, '_blank');
