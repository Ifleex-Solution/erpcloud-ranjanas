...
class Invoice extends MX_Controller
{
    ...
    function bdtask_invoice_form()
    {
        ...
        $data['password']         = display('add_invoice');

        // $data['taxes']         = $this->invoice_model->tax_fileds();
        $data['module']        = "invoice";

        $id2 = $this->session->userdata('id2');
    ...
    public function checkUser()
    {
        ...
        $user = $this->checkUsers($this->session->userdata('email'), $this->input->post('password', TRUE));
    ...
    {
        $query = $this->db->select('count(*) as user_count')
            ->from('users a')
            ->join('user_login b', 'b.user_id = a.user_id')
            ->where('b.username', $email)
            ->where('b.password', md5('gef' . $password))  // Include the password check
            ->where('b.status', 1)
            ->get();

        // Retrieve the count as an integer
    ...
    {
        if ($this->session->userdata('id2') === "4") {
            $incremented_id = $this->number_generator();
            ...
            $datainv = array(
                ...
                'status'          => 1,
                'employee_id' => $this->session->userdata('id2'),
    ...
    public function reprint()
    {
        ...
        $printdata       = $this->invoice_model->bdtask_invoice_pos_print_direct($invoice_id, "a");

        $data['details'] = $this->load->view('invoice/pos_print', $printdata, true);
        echo json_encode($data);
    }

    public function reprintgod()
    {
        $invoice_id = $this->input->post('invoice_id', TRUE);

        $printdata       = $this->invoice_model->bdtask_invoice_pos_print_direct($invoice_id, "god");
    ...
    SELECT 
        ...
        pc.category_name AS productname,
        id.employeeId AS empId, 
        emp.first_name AS sb,
...
ROUND(id.quantity, 0) AS qty,
    ...
    {
        if ($this->session->userdata('id2') === "8") {
            ...
            echo json_encode("Invalid UserId");
        }
    ...
    public function getInvoiceSummary()
    {
        ...
        // $this->db->where('a.employee_id =',  $this->session->userdata('id2'));
    ...
    public function invoice_setup()
    {
        ...
        $product_information = $this->db->select('*')
            ...
            ->join('product_category pc', 'pc.category_id = pi.category_id')
        ...
        if ($product_information != null) {
            ...
            $data2 = (object) array(
                ...
                'product_name'   => $product_information->category_name,
    ...
    public function number_emp_generator()
    {
        ...
        if ($invoice_no != '') {
            ...
            $invoice_no = 325911001;
        }
        return $invoice_no;
    }

    public function empNextNumber()
    {
        $this->db->select_max('invoice_id', 'invoice_no');
        $query      = $this->db->get('emp');
        $result     = $query->result_array();
