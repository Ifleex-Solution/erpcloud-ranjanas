...
class Invoice_model extends CI_Model
{
    ...
    public function getInvoiceList($postData = null, $empid = null, $sl = null)
    {
        ...
        foreach ($records as $record) {
            ...
            </button>  <button class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="Reprint" 
                onclick="reprintInvoice(' . $record->id . ', \'' . $type . '\')">
                <i class="fa fa-fax" ></i>
            </button>';
            } else {
                $button .= '
...
');
        $this->db->from('invoice a');
        $this->db->join('invoice_details c', 'c.invoice_id = a.id');
        $this->db->join('employee_history b', 'b.id = c.employeeId', 'left');
        ...
        $this->db->order_by('invoice_id1', 'ASC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
