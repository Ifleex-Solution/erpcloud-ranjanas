...
class Product extends MX_Controller
{
    ...
    public function bdtask_product_form($id = null)
    {
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (!empty($id)) {
                ...
                $result3 = $query3->row();
                $data['productId'] =  $result3->highest_product_id ;
            }
            ...
            if (($handle = fopen($_FILES['upload_csv_file']['tmp_name'], 'r')) !== FALSE) {
                ...
                while ($csv_line = fgetcsv($fp, 1024)) {
                    ...
                    for ($i = 0, $j = count($csv_line); $i < $j; $i++) {
                        ...
                        $insert_csv = array();
                        $insert_csv['employee_zktec_id'] = $csv_line[1] ;
                        $insert_csv['date'] =date('Y-m-d', strtotime($csv_line[2]));


                        $insert_csv['sign_in'] = date('Y-m-d H:i:s', strtotime( $csv_line[2]." ".$csv_line[3]));
                        $insert_csv['sign_out'] = date('Y-m-d H:i:s', strtotime(  $csv_line[2]." ".$csv_line[4])) ;
                        $insert_csv['staytime'] = $csv_line[5];
                    }
                    $count++;
                    if ($csv_line[0] != "EMP Name") {
                        $supp_prd = array(
                            'employee_zktec_id'    => $insert_csv['employee_zktec_id'],
                            'date' => $insert_csv['date'],
                            'sign_in' => $insert_csv['sign_in'],
                            'sign_out' => $insert_csv['sign_out'],
                            'staytime' => $insert_csv['staytime']
                        );

                        $this->db->insert('attendance', $supp_prd);
                    ...
                    //     $product = array(
                    //         'productId'    =>  str_pad($insert_csv['product_id'], 6, '0', STR_PAD_LEFT),
                    //     );
                    //     $this->db->update('productsequence', $product);
                    // }
    ...
    // bar code part
    public function barcode_tag()
    {
        $data = array(
            'title'           => "Barcode Tag",
        );
        $data['module']        = "product";
        $data['page']          = "barcode_print_page2";
        echo modules::run('template/layout', $data);
    }
    ...
    public function printtag()
    {
        ...
        $this->db->query('TRUNCATE TABLE barcode_tag');
        foreach ($labels as $label) {
            ...
            for ($i = 0; $i < $noOfLabel; $i++) {
                ...
                $this->db->insert('barcode_tag', $supp_prd);
            }
        }
    ...
    public function insertLabelPrint()
    {
        $supp_prd = array(
            ...
            'description' => $this->input->post('productName'),
            'pgroup' => $this->input->post('categoryName'),
            'pbrand' => $this->input->post('brandcodeName'),
    ...
    public function getProductById()
    {
        ...
        $this->db->where('a.product_id', $this->input->post('id'));
        $query = $this->db->get();
    ...
    public function getLabelPrintData()
    {
        $query = $this->db->select('id, no_of_labels as noOfLabel, product_code as productId, 
                                description as productName, pgroup as categoryName, 
                                pbrand as brandcodeName, price as price')
            ->from('labelprint')
            ->get();
